...
get_tmp_dir() {
  mktemp -d -t "${1}_$(date +%Y-%m-%d-%H-%M-%S)-XXXXXXXXXXXX"
}
...
function quit_pb_option() {
  printf "\n\nThe config.yml should contain 7 parameters.\n"
  ...
  printf "\noutsummary: absolute path of the log file containing the input validation summary."
...
check_fasta() {
  local fasta=$1
  local nbseq
  nbseq=$(awk '/^>/ {count++} END {print count + 0}' "$fasta")
  if ((nbseq == 0)); then
    printf 'Problem: no sequence found in FASTA file %s\n' "$fasta" >&2
    return 1
  fi
  printf '%s\n' "$nbseq"
}
...
## control of gff
nb_gene=$(awk -F '\t' '$3 == "gene" {count++} END {print count + 0}' "$ref_gff")
nb_cds=$(awk -F '\t' '$3 == "CDS" {count++} END {print count + 0}' "$ref_gff")
nb_exon=$(awk -F '\t' '$3 == "exon" {count++} END {print count + 0}' "$ref_gff")

pb_gff=0
if (($nb_gene == 0)); then
  pb_gff=1
  printf "ERROR: no gene feature found in gff file ${ref_gff}.\n" >&2
fi
if (($nb_cds == 0)); then
  pb_gff=1
  printf "ERROR: no CDS feature found in gff file ${ref_gff}.\n" >&2
fi
if (($nb_exon == 0)); then
  printf "WARNING: no exon feature found in gff file ${ref_gff}; CDS features will be used instead.\n" >&2
fi

if ((pb_gff == 1)); then exit 1; fi

## control of chromosome between reference fasta and gff
grep "^>" $ref_genome | cut -d " " -f 1 | sed 's/>//g' | uniq >$tmpdir/chrnames.tmp
awk -F '\t' '!/^#/ {print $1}' "$ref_gff" | sort | uniq >$tmpdir/chrnamesgff.tmp
