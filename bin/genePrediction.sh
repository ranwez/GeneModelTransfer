...
blastbest=(($covblast*0.6+($blastForBest/100)*0.4 |bc -l))
cdnabest=(($covcdna*0.6+($cdna2genomeForBest/100)*0.4 |bc -l))
protbest=(($covprot*0.6+($prot2genomeForBest/100)*0.4 |bc -l))
if [ $mode == "first" ] 
then
...
then
	if (( $(echo "$blastForBest > $prot2genomeForBest" |bc -l) )) && (( $(echo "$blastForBest > $cdna2genomeForBest" |bc -l) )) && [ -s mapping_LRRlocus_${SPECIES}.gff ] 
	then 
	echo "-------------------------------------blastcds"
