...
function run_exonerate() {
  ...
  # If exonerate failed, check if the error is allowed or if the script should keep going (if IGNORE_EXONERATE_ERRORS == true)
  if [[ $? -ne 0 ]]; then
    ...
    echo "Exonerate encountered ${error_type} error:"
    echo "$error_message" | head -1
    echo "When running:"
    echo "$cmd"
    if [[ "$IGNORE_EXONERATE_ERRORS" == "true" && $error_type != "known" ]]; then
      echo "But IGNORE_EXONERATE_ERRORS is set to TRUE."
    elif [[ "$error_type" == "unexpected" ]]; then
      exit_code=1
    fi
