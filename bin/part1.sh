...
extractSeq $CDS
for file in *; do mv "$file" "OSJnip_$file"; done
...
chmod 777 ./REF_CDS/*
          ...
          #------------------------------------------#
mmseqs createdb $2 $datadir/$1_genome_db 
mmseqs createdb $5 $datadir/prot_db 
mmseqs search $datadir/prot_db $datadir/$1_genome_db $datadir/resultDB_aln.m8 tmp -s 8.5 -a -e 0.1 --min-length 10 --merge-query 1 --cov-mode 2 --max-seqs 30000 --sequence-overlap 1000 
mmseqs convertalis $datadir/prot_db $datadir/$1_genome_db $datadir/resultDB_aln.m8 $datadir/res_candidatsLRR_in_$SPECIES.out --format-output query,target,qlen,alnlen,qstart,qend,tstart,tend,nident,pident,gapopen,evalue,bits  
cat $datadir/res_candidatsLRR_in_$SPECIES.out > $directory/res_candidatsLRR_in_$SPECIES.out

#mmseqs createdb $2 ./$1_genome_db 
#mmseqs createdb $5 ./prot_db 
#mmseqs search ./prot_db ./$1_genome_db ./resultDB_aln.m8 tmp -s 8.5 -a -e 0.1 --min-length 10 --merge-query 1 --cov-mode 2 --max-seqs 30000 --sequence-overlap 1000 
#mmseqs convertalis ./prot_db ./$1_genome_db ./resultDB_aln.m8  res_candidatsLRR_in_$SPECIES.out --format-output query,target,qlen,alnlen,qstart,qend,tstart,tend,nident,pident,gapopen,evalue,bits  
...
gawk 'BEGIN{OFS="\t"}{if($10>=65){print($0)}}' $datadir/res_candidatsLRR_in_$SPECIES.out | sort -k1,2 -Vk7,7 > sort_65_candidatsLRR_in_$SPECIES.out
...
##------------------------------
gawk 'BEGIN{OFS="\t"}{if($10>=45 && $10<65){print($0)}}' $datadir/res_candidatsLRR_in_$SPECIES.out | sort -k1,2 -Vk7,7 > sort_45_candidatsLRR_in_$SPECIES.out
...
cat liste_query_target.txt > liste_query_target
DIR=$(pwd)
...
python3 $directory/Extract_sequences_from_genome.py -f $BLASTDB -g filtered_candidatsLRR_in_$SPECIES.gff -o ./DNA_candidatsLRR_in_$SPECIES.fasta  -t gene 
