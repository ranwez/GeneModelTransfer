...
class HspOverlapCacher:
    ...
    def from_hsp(cls, hsp: HSP) -> "HspOverlapCacher":
        return cls(hsp_overlaps=cls.pre_compute_overlap(hsp), nb_hsp=len(hsp))
    ...
    def pre_compute_overlap(cls, hsps: list[HSP]) -> list[HspCompatibleOverlap]:
        ...
        for i in range(1, n):
            ...
            for j in range(i):
                hspj = hsps[j]
                if (
                    (hspj.prot_bounds.start > hspi.prot_bounds.start)
                    and hspj.locS_bounds.start > hspi.locS_bounds.start
                ) or (
                    (hspj.prot_bounds.start < hspi.prot_bounds.start)
                    and hspj.locS_bounds.start < hspi.locS_bounds.start
                ):
                    loc_overlap = math.ceil(
                        hspi.locS_bounds.overlap(hspj.locS_bounds) / 3
                    )
                    ...
                    hspij_min_length = min(
                        hspi.prot_bounds.length(),
                        hspj.prot_bounds.length(),
                        hspi.locS_bounds.length() // 3,
                        hspj.locS_bounds.length() // 3,
                    )
                    if diff_overlap > 0.5 * hspij_min_length:
                        continue
                    max_id_overlap = min(
                        max(
                            loc_overlap, prot_overlap
                        ),  # max hsp overlap with locus and prot
                        hspj.nident,  # Identities in hsp_j
                        hspi.nident,  # Identities in hsp_i
                    )
                    hsp_overlaps[row_idx + j] = HspCompatibleOverlap(
                        True, max_id_overlap
                    )
        return hsp_overlaps
...
class CandidateLocus:
    ...
    def compute_score(
        self, protInfo: GeneInfo, max_intron_len: int, params: ParametersLociScoring
    ) -> float:
        ...
        similarity = (params.identityWeigthvsCoverage * self.nident + max_homology) / (
            params.identityWeigthvsCoverage + 1
        )
        ...
        if protInfo != None:
            ...
            length_deviation = (
                (self.chr_bounds).length() - prot_genomic_len
            ) / max_intron_len
            length_penalty_in_percent = max(
                -1, len_penalty_percentage * (1 - max(1, math.exp(length_deviation)))
            )
        else:
    ...
    def as_gff(self) -> str:
        ...
        prot_path_info = (
            ",".join(f"{bound.start}-{bound.end}" for bound in self.prot_path)
            if self.prot_path
            else "None"
        )
...
class MergeableHSP:
    ...
    def mergeHSP(self, hsp: HSP) -> bool:
        if (
            self.prot_id == hsp.prot_id
            and hsp.locS_bounds.start - self.max_coord <= self.max_intron_len
        ):
    ...
    def max_possible_coverage(self) -> float:
        if len(self.hsps) == 1:
            ...
            for hsp in self.hsps:
                homolog_fraction.add(
                    [(hsp.prot_bounds.start, (hsp.prot_bounds.end + 1))]
                )
    ...
    def compute_candidate_loci_rec(self) -> list[CandidateLocus]:
        ...
        for hsp_list in (HSPl, HPSr):
            if len(hsp_list) > 0:
                sub_region = MergeableHSP(
                    self.prot_id, hsp_list, self.max_coord, self.max_intron_len
                )
    ...
    def compute_candidate_loci(self) -> CandidateLocus:
        ...
        for j, hspj in enumerate(self.hsps):
            ...
            for i in reversed(range(j)):
                ...
                if (
                    nident[i] + nident[j] < best_ident
                ):  # overlap only decrease score so no need to compute them
                ...
                nident_ij = (
                    nident[i] + nident[j] - path_compatible_overlap.max_id_overlap
                )
            ...
            nident[j] = best_ident
            best_prev_nident[j] = (
                max(best_prev_nident[j - 1], nident[j]) if j >= 1 else nident[j]
            )
        # Find the index of the best scoring HSP
        best_idx = argmax(nident)
        candidate_locus = CandidateLocus.from_hsp_path(
            [self.hsps[i] for i in paths[best_idx]], nident[best_idx]
        )
...
def path_bound(path: list[Bounds]) -> Bounds:
    return Bounds(min(path[0].start, path[-1].start), max(path[0].end, path[-1].end))


def small_shrinking(
    locus: CandidateLocus, covered: InterLap, ntShrink: int
) -> tuple[bool, bool]:
    ...
    if locus.chr_bounds.length() < 10 * ntShrink:
        return (False, False)
    if not covered.__contains__(
        (locus.chr_bounds.start + ntShrink, locus.chr_bounds.end - ntShrink)
    ):
        if not covered.__contains__(
            (locus.chr_bounds.start + ntShrink, locus.chr_bounds.end)
        ):
            locus.chr_bounds.start += ntShrink
            return (True, False)
        if not covered.__contains__(
            (locus.chr_bounds.start, locus.chr_bounds.end - ntShrink)
        ):
...
def keep_best_non_overlaping_loci(
    candidate_loci: list[CandidateLocus], params: ParametersLociScoring
) -> list[CandidateLocus]:
...
def set_desired_expansion(
    candidate_loci: list[CandidateLocus],
    expand_params: ParametersExpansion,
    protInfo: dict,
) -> None:
    ...
    for locus in candidate_loci:
        ...
        locus.expansion = Bounds(
            expand_params.nb_nt_default, expand_params.nb_nt_default
        )
        if locus.prot_bounds.start > expand_params.nb_aa_for_missing_part:
            ...
            if gff_file is not None:
                ...
                locus.expansion.start = max(
                    locus.expansion.start, int(template_prot_info.longest_intron * 1.1)
                )
        if (
            locus.prot_len - locus.prot_bounds.end
            > expand_params.nb_aa_for_missing_part
        ):
        ...
        if gff_file is not None:
            ...
            if locus.prot_bounds.start > 0:
                (gene_start, locus_start, _gene_end) = cds_info.get_genomic_coord(
                    locus.prot_bounds.start
                )
...
def expands(
    ...
    if candidate_loci[0].expansion.start > 0:
        candidate_loci[0].chr_bounds.start = max(
            0, candidate_loci[0].chr_bounds.start - candidate_loci[0].expansion.start
        )
    ...
    for loc_id in range(1, len(candidate_loci)):
        ...
        available_nt_for_expansion = (
            locus.chr_bounds.start - prev_locus.chr_bounds.end - 1
        )
        if (
            available_nt_for_expansion
            >= prev_locus.expansion.end + locus.expansion.start
        ):
            ...
            if prev_locus.shrink_info[1]:
                prev_locus.chr_bounds.end += available_nt_for_expansion
            elif locus.shrink_info[
                0
            ]:  # only the locus with the lowest score can be shrinked on this interval
                locus.chr_bounds.start -= available_nt_for_expansion
            else:
                nt_for_prev = math.floor(
                    available_nt_for_expansion
                    * prev_locus.expansion.end
                    / (locus.expansion.start + prev_locus.expansion.end)
                )
                prev_locus.chr_bounds.end += nt_for_prev
                locus.chr_bounds.start -= available_nt_for_expansion - nt_for_prev


def find_candidate_loci_from_hsps(
    list_hspchr: list[HSP_chr],
    protInfo: dict,
    default_intron_lg: int,
    ...
    list_hspchr.append(
        HSP_chr("dummmyyChr", "", [])
    )  # add a dummy HSP_chr to make sure the last chromosome is processed
    ...
    for hsp_chr in list_hspchr:
        ...
        if prev_chr is None or prev_chr != hsp_chr.chr_id:
            if prev_chr is not None:
                ...
                candidate_loci_per_chr[prev_chr] = keep_best_non_overlaping_loci(
                    chr_candidate_loci, params.loci_scoring
                )
                if params.expansion is not None:
                    print("start expanding")
                    expands(
                        candidate_loci_per_chr[prev_chr], params.expansion, protInfo
                    )
        ...
        hsps.append(
            HSP.build_dummy("dummmyProt")
        )  # add a dummy HSP to make sure the last HSP is processed
            ...
            if mergeableHSP is None or not mergeableHSP.mergeHSP(hsp):
                if mergeableHSP is not None:
                    ...
                    for candidate_locus in candidate_loci:
                        candidate_locus.compute_score(
                            protInfo[mergeableHSP.prot_id],
                            mergeableHSP.max_intron_len,
                            params.loci_scoring,
                        )
                        if candidate_locus.score > params.loci_scoring.min_score and (
                            candidate_locus.pc_similarity
                            >= params.loci_scoring.min_similarity
                        ):  # or candidate_locus.nhomol_prot>500)):
                            chr_candidate_loci.append(candidate_locus)
                if hsp.prot_id != "dummmyProt":
                    if protInfo.keys().__contains__(hsp.prot_id):
                        longest_allowed_intron = max(
                            default_intron_lg,
                            int(protInfo[hsp.prot_id].longest_intron * 1.1),
                        )
                    else:
                        continue  # skip HSPs with unknown protein for tests
                    mergeableHSP = MergeableHSP(
                        hsp.prot_id, [hsp], hsp.locS_bounds.end, longest_allowed_intron
                    )
...
def find_candidate_loci(gff_file: str, blast_file: str, params=None, chr=None) -> dict:
    ...
    (protInfo, def_intron_lg) = gff_to_geneInfo(
        gff_file, candideLociParams.hsp_clustering.quantileForMaxIntronLength
    )
    ...
    candidateLoci = find_candidate_loci_from_hsps(
        list_hspchr, protInfo, def_intron_lg, candideLociParams
    )
    return candidateLoci


#############################
def add_loci_from_mergeableHSPs(
    mergeableHSP: MergeableHSP,
    protInfos: dict,
    params: ParametersCandidateLoci,
    candidateLoci: [CandidateLocus],
) -> None:
    ...
    for candidate_locus in candidate_loci:
        candidate_locus.compute_score(
            protInfo, mergeableHSP.max_intron_len, params.loci_scoring
        )
        if candidate_locus.score > params.loci_scoring.min_score and (
            candidate_locus.pc_similarity >= params.loci_scoring.min_similarity
        ):  # or candidate_locus.nhomol_prot>500)):
            candidateLoci.append(candidate_locus)


def init_mergeagleHSPs(
    hsp: HSP, protInfo: dict, default_intron_lg: int, params: ParametersCandidateLoci
) -> MergeableHSP:
    if protInfo.keys().__contains__(hsp.prot_id):
        longest_allowed_intron = max(
            default_intron_lg, int(protInfo[hsp.prot_id].longest_intron * 1.1)
        )
    else:
...
def find_candidate_loci_from_file(
    gff_file: str, sorted_blast_file: str, params=None, chr=None
) -> dict:
    if params is None:
        candideLociParams = ParametersCandidateLoci()
    else:
    ...
    with open(sorted_blast_file, "r") as file:
        reader = csv.reader(
            file, delimiter="\t"
        )  # Use "," if it's a comma-separated file
        candidate_loci_per_chr = {}
        prev_chr: Optional[str] = None
        ...
        for row in reader:
            ...
            if prev_chr != hsp.chr_id:
                ...
                if candideLociParams.expansion is not None:
                    expands(
                        candidate_loci_per_chr[prev_chr],
                        candideLociParams.expansion,
                        protInfo,
                    )
                chr_candidate_loci = []
            (prev_chr, prev_strand) = (hsp.chr_id, hsp.strand)
