...
parser.add_argument(
    ...
    "--type",
    type=str,
    choices=["gene", "cdna", "prot", "FScdna", "FSprot", "cds"],
    help="type of extracting sequence : <gene> extract genomique dna sequence with intron, <cdna> extract reconstruct coding nucleotide sequence with frameshift, <prot> extract reconstruct protein sequence, <exons> extract isolated coding exons, <cds> extract reconstruct coding nucleotide sequence without frameshift.",
)
parser.add_argument(
    "-m",
    "--margin",
    type=positive_int,
    default=0,
    help="Margin size (positive integer) to include flanking regions around gene extracted sequences. Default is 0.",
)
parser.add_argument(
    "-a",
    "--no_FS_codon",
    ...
    for row in gff_reader:
        ...
        if row[2] == "gene":
            ...
            begin = max(0, begin)  # Ensure begin is not negative
                ...
                end, len(str(chr_dict[row[0]].seq))
            )  # Ensure end does not exceed sequence length
        ...
        if row[2] == "gene":
            if len(dna) > 0:
                ...
                if typeseq == "FSprot":
                    if len(dna) % 3 != 0:
                        print(
                            "Warning: Partial codon, len(sequence) not a multiple of three:"
                        )
                        print(sid)
                        print(dna)
