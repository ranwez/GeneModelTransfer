...
def add_classification_with_lists(blast_df, threshold):
    ...
    """
    # Step 1: Sort by prot_id and loc_startS
    sorted_df = blast_df.sort(["prot_id", "loc_startS"])
...
def add_classification_slower(blast_df, threshold):
    """
    Add class_id, class_id_startS, and class_id_endS columns to the BLAST DataFrame.
    Optimized version that dynamically creates class information during iteration.

    Args:
        blast_df (pl.DataFrame): Input DataFrame with HSPs.
        threshold (int): Maximum distance allowed between loc_startS and the current class's loc_endS.

    Returns:
        pl.DataFrame: Enriched DataFrame with class_id, class_id_startS, and class_id_endS.
    """
    # Step 1: Sort by prot_id and loc_startS
    sorted_df = blast_df.sort(["prot_id", "loc_startS"])

    # Step 2: Initialize variables for dynamic class tracking
    class_ids = []      # List to store class IDs
    class_startS_list = []  # List to store startS of each class
    class_endS_list = []    # List to store endS of each class
    current_class_id = 1    # Start with class 1
    class_startS = None     # Start of the current class
    class_endS = -float("inf")
    prev_prot_id = None
    # get indices to speed up iteration
    col_indices = {name: idx for idx, name in enumerate(sorted_df.columns)}
    prot_id_idx = col_indices["prot_id"]
    loc_startS_idx = col_indices["loc_startS"]
    loc_endS_idx = col_indices["loc_endS"]

    # Step 3: Process rows one-by-one
    for row in sorted_df.iter_rows(named=False):
        row_prot_id = row[prot_id_idx]
        row_startS = row[loc_startS_idx]
        row_endS = row[loc_endS_idx]
        if class_startS is None:
            class_startS = row_startS
        if (
            prev_prot_id is not None and  # HSP of the same prot
            row_prot_id == prev_prot_id and
            # HSP nearby on the genome compatible with intron length
            row_startS - class_endS <= threshold
        ):    # Stay in the current class: Update the endS
            class_endS = max(class_endS, row_endS)
        else:
            # Finalize the current class
            class_startS_list.append(class_startS)
        ...
        # Assign the current class ID to the row
        class_ids.append(current_class_id)
        prev_prot_id = row_prot_id

    # Step 4: Handle the last class (finalize it)
    class_startS_list.append(class_startS)
    class_endS_list.append(class_endS)
...
def add_simplified_coord(blast_full):
    """
    Enriches the input DataFrame with:
    1. `strand`: Determines strand based on loc_start and loc_end.
    2. `chr_max_coord`: Max loc_start and loc_end per chr_id.
    3. `loc_startS` and `loc_endS`: Simplified coordinates based on strand.

    Args:
        blast_full (pl.DataFrame): The input DataFrame containing 'loc_start', 'loc_end', and 'chr_id'.

    Returns:
        pl.DataFrame: Enriched DataFrame with added columns.
    """
...
def main():
    ...
    # add strand, loc_startS and loc_endS column : simplified coordinates for loci that work both in reverse and forward for interval merging
    ...
    # Filter the DataFrame into positive and negative strands
    blast_neg = blast_full2.filter(
        pl.col("strand") == -1
    ).sort(["prot_id", "loc_startS"])
    print("------- blast NEG ---------")
    print(blast_neg.head())

    blast_pos = blast_full2.filter(
        pl.col("strand") == 1
    ).sort(["prot_id", "loc_endS"])

    print("+++++++ blast POS +++++++++")
    print(blast_pos.head())

    # TODO check if blast is empty
    # blast_neg = add_classification_with_lists(blast_neg, 4000)
    blast_pos = add_classification_with_lists(blast_pos, 4000)
    print(blast_pos.head())
    print("done")
